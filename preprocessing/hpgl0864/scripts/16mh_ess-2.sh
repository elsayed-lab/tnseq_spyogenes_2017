...
#SBATCH --job-name=mh_ess-2
#SBATCH --mem=6G
#SBATCH --cpus-per-task=1
#SBATCH --output=/cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864/outputs/mh_ess-2.sbatchout

echo "####Started /cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864/scripts/16mh_ess-2.sh at $(date)" >> outputs/log.txt
cd /cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864 || exit

# Run mh-ess using the min hit: 2
...
gumbelMH.py -f outputs/essentiality/hpgl0864-v0M1_gene_tas.txt -m 2 -s 1000 \
  1>outputs/essentiality/mh_ess-hpgl0864-v0M1_gene_tas_m2.csv \
  2>outputs/essentiality/mh_ess-hpgl0864-v0M1_gene_tas_m2.err

## The following lines give status codes and some logging
echo $? > outputs/status/mh_ess-2.status
echo "###Finished ${SLURM_JOBID} 16mh_ess-2.sh at $(date), it took $(( SECONDS / 60 )) minutes." >> outputs/log.txt
