...
#SBATCH --job-name=btv0M1_mgas_5448_stats
#SBATCH --mem=6G
#SBATCH --cpus-per-task=4
#SBATCH --output=/cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864/outputs/btv0M1_mgas_5448_stats.sbatchout

echo "####Started /cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864/scripts/12btv0M1_mgas_5448_stats.sh at $(date)" >> outputs/log.txt
...
reads_tmp=$(grep "^# reads processed" outputs/bowtie_mgas_5448/hpgl0864-v0M1.err | awk -F: '{print $2}' | sed 's/ //g')
reads=${reads_tmp:-0}
one_align_tmp=$(grep "^# reads with at least one reported" outputs/bowtie_mgas_5448/hpgl0864-v0M1.err | awk -F": " '{print $2}' | sed 's/ .*//g')
one_align=${one_align_tmp:-0}
failed_tmp=$(grep "^# reads that failed to align" outputs/bowtie_mgas_5448/hpgl0864-v0M1.err | awk -F": " '{print $2}' | sed 's/ .*//g')
failed=${failed_tmp:-0}
sampled_tmp=$(grep "^# reads with alignments sampled" outputs/bowtie_mgas_5448/hpgl0864-v0M1.err | awk -F": " '{print $2}' | sed 's/ .*//g')
sampled=${sampled_tmp:-0}
rpm_tmp=$(perl -e "printf(1000000 / ${one_align})" 2>/dev/null)
rpm=${rpm_tmp:-0}
stat_string=$(printf "hpgl0864,v0M1,%s,%s,%s,%s,%s,%s,hpgl0864-v0M1.count.xz" "${original_reads}" "${reads}" "${one_align}" "${failed}" "${sampled}" "$rpm")
echo "$stat_string" >> outputs/bowtie_stats.csv
## The following lines give status codes and some logging
echo $? > outputs/status/btv0M1_mgas_5448_stats.status
echo "###Finished ${SLURM_JOBID} 12btv0M1_mgas_5448_stats.sh at $(date), it took $(( SECONDS / 60 )) minutes." >> outputs/log.txt
