...
#SBATCH --job-name=xzal
#SBATCH --mem=6G
#SBATCH --cpus-per-task=1
#SBATCH --output=/cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864/outputs/xzal.sbatchout

echo "####Started /cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864/scripts/11xzal.sh at $(date)" >> outputs/log.txt
cd /cbcb/nelsayed-scratch/atb/tnseq/spyogenes_5448v3/preprocessing/hpgl0864 || exit

## Compressing the sequences which successfully aligned against /cbcbhomes/abelew/libraries/genome/indexes/mgas_5448 using options --best -v 1 -M 1.
nice -n 20 xz -f -9e ././   2>./xzal.log 1>&2
## The following lines give status codes and some logging
echo $? > outputs/status/xzal.status
echo "###Finished ${SLURM_JOBID} 11xzal.sh at $(date), it took $(( SECONDS / 60 )) minutes." >> outputs/log.txt
